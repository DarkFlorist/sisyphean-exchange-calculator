...
export interface BaseInputModel extends MappedOmit<InputHTMLAttributes<HTMLInputElement>, 'value' | 'onInput' | 'onInput'> {
...
export function Input<T>(model: UnparsedInputModel | ParsedInputModel<T>) {
	if (model.tryParse) {
		...
		return <ParsedInput {...model} value={model.value} sanitize={model.sanitize || (x => x)} tryParse={value=>({ok: true, value})} serialize={x=>x||''}/>
	}
}
